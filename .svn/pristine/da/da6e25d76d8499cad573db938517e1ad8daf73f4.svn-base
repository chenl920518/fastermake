					...
					if (result.state == 0) {
						...
						if (noteMessageList && noteMessageList.length > 0) {
							...
							for (var i = 0; i < noteMessageList.length; i++) {
								...
								if (noteMessage.isRead == 0
									...
									tempHtml += '<tr class="wd" receiver_id="'
											...
											+ (noteMessage.dialogueId ? noteMessage.dialogueId
													: "")
											+ '">'
											+ '<td><i class="iconfont color_006dcc">&#xe61b;</i></td>'
								} else {
									tempHtml += '<tr id="'
											+ noteMessage.id
											+ '" receiver_id="'
								...
								if (receiverId && receiverId != factoryId
										&& receiverInfo) {
									tempHtml += '<td><span class="w140">'
											+ (receiverInfo.factoryName ? receiverInfo.factoryName
													: "") + '</span></td>'
								} else if (sendId && sendId != factoryId
										&& sendInfo) {
									tempHtml += '<td><span class="w140">'
									...
									tempHtml += '<td><span class="w140">系统消息</span></td>'
								}
						...
						$('#messageTable tr').click(
								function() {

									$('.page1').hide()

									$(this).removeClass('wd').find('i')
											.removeClass('color_006dcc').html(
													'&#xe60f;')

									var dialogueId = $(this).attr('dialogueid')
									var id = $(this).attr('id')
									var receiverId = $(this)
											.attr('receiver_id')
