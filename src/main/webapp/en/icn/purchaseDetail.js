			 ...
			 function(result){
			      if(result.state == 0){
			    	 ...
			    	 if(quoteInfo.orderStatus == 0){
			    		 ...
			    		 $('.panel4').hide();
			    		 $('#quoteId').next().css({"color":"#ED1C24"}).text('Not Passed'); 
			    	 }
		    		...
		    		if(strs.length != 0){
			    	 ...
			    	 var flag2 = false;    //判断是否有第三报价数
							 ...
							 var tr2 = '<tr class="trcol currdis">'+
							  ...
							  var edit_tr1 = '<tr>'+
						                     '<td>'+
					                             ...
					                             '</form>'+
					                          '</td>'+
					                          '<td> <textarea class="form-control w120" onfocus="w120_edit_focus(this)" onblur="w120_edit_blur(this)">'+(products[j].quantityList == null ? '' : products[j].quantityList)+'</textarea> </td>'+
					                          ...
					                          '<td><textarea class="form-control w120" onfocus="w120_edit_focus(this)" onblur="w120_edit_blur(this)">'+(products[j].targetPrice == null ? '' : products[j].targetPrice)+'</textarea></td>'+
					   ...
					   for(var k=0;k<supplierQuoteInfos.length;k++){
						   ...
						   var a = '<a href="/zh/purchase_big_goods.html?factoryId='+supplierQuoteInfos[k].factoryId+'&orderId='+supplierQuoteInfos[k].orderId+'" target="_blank">Carry out big production</a>';
						   if(supplierQuoteInfos[k].quoteStatus == 7){
							   a = '<a href="#" class="btn disabled" role="button" >Carry out big production</a>';
						   }else if(supplierQuoteInfos[k].quoteStatus == 2){
							   a = '<a href="/zh/purchase_big_goods_generation.html?factoryId='+supplierQuoteInfos[k].factoryId+'&orderId='+supplierQuoteInfos[k].orderId+'" target="_blank">View large production</a>';
							   ...
							   a = '<a href="/zh/purchase_big_goods_report.html?factoryId='+supplierQuoteInfos[k].factoryId+'&orderId='+supplierQuoteInfos[k].orderId+'"  target="_blank">View large production</a>';
						   ...
						   var tr = ' <tr>'+
		                                '<td>'+
		                                    '<h6 class="h110"><a href="/zh/view_factory_info.html?id='+supplierQuoteInfos[k].factoryId+'" target="_blank" class="h_over h_over1 h130 h110">'+(supplierQuoteInfos[k].factoryName == null ? '' : supplierQuoteInfos[k].factoryName)+'</a></h6>'+
		                                    '<i>'+(supplierQuoteInfos[k].createTime == null ? '' :(new Date(supplierQuoteInfos[k].createTime)).Format("yyyy-MM-dd"))+'</i>'+
		                                '</td>'+
		                                '<td><em class="h130">'+supplierQuoteInfos[k].totalAmount+'CNY</em></td>'+
		                                '<td><em>'+supplierQuoteInfos[k].validityDays+'days</em></td>'+
		                                ...
		                                '<td>'+
		                                    ...
		                                    '<a href="###" class="btn disabled" role="button">Proposed factory inspection requirements</a>'+
		                                    '<a href="###" class="btn disabled" role="button">Proofing request</a>'+a+		                                    
		                                '</td>'+
		                            '</tr>';
				         ...
				         if(supplierQuoteProductss != null && supplierQuoteProductss != ''){
								 ...
								 var compare_div  =  '<div class="compare clearfix">'+
									                 		'<ul class="pull-left">'+
									                 			'<li>'+
									                 				'<a href="/zh/view_factory_info.html?id='+supplierQuoteInfos[k].factoryId+'">'+(supplierQuoteInfos[k].factoryName == null ? '' : supplierQuoteInfos[k].factoryName)+'</a>'+
									                 			'</li>'+product_li+						                 			
									                 			'<li>Total（CNY）</li>'+
									                 		'</ul>'+
									                 		'<table>'+
					   ...
					   for(var m=0;m<factorys.length;m++){				   
							...
							var tr=   ' <tr>'+
					                        '<td>'+
					                        '<h6><a href="/zh/view_factory_info.html?id='+factorys[m].factoryId+'" target="_blank" class="w110 h_over">'+(factorys[m].factoryName == null ? '' : factorys[m].factoryName)+'</a></h6>'+
						                    '</td>'+
...
function edit_product(obj){
	...
	$('#product_tbody2 tr').each(function(i){
		...
		if(tl > 2){
			...
			var quantityList = $(this).find('td').eq(2).find('textarea').val();
			var quantityUnit = $(this).find('td').eq(3).find('textarea').val();
			var targetPrice = $(this).find('td').eq(4).find('textarea').val();
			var materials = $(this).find('td').eq(5).find('input').val();
			var weight = $(this).find('td').eq(6).find('input').val();
